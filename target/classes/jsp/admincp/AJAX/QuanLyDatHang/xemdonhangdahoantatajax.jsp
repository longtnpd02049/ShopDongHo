...
<div class="row">
	<div class="col-lg-12">
		...
		<div class="panel-body">
					...
					<c:if test="${ListDatHang.size()>0}">
						<c:forEach items="${ListDatHang}" var="item">
							<tr class="odd gradeX">
									...
									<c:if test="${role.equalsIgnoreCase('Administrator')}">
										<td>
											<c:if test="${item.status.equalsIgnoreCase('ok')}">
									...
									<c:choose>
										<c:when test="${item.status.equalsIgnoreCase('wait')}">
											<td></td>
											<td></td>
										</c:when>
									...
									<td>
										<c:choose>
											<c:when test="${item.status.equalsIgnoreCase('wait')}">
												<img src="../images/erroricon.png" width="24" height="24"><br/>Chưa xác nhận
											</c:when>
											<c:when test="${item.status.equalsIgnoreCase('ok')}">
												<img src="../images/Semi-success-icon.png" width="24" height="24"><br/>Đã xác nhận
											</c:when>
											<c:when test="${item.status.equalsIgnoreCase('success')}">
												<img src="../images/successicon.png" width="24" height="24"><br/>Đã hoàn tất
											</c:when>
										</c:choose>
									</td>
							</tr>
			...
			<c:if test="${role.equalsIgnoreCase('Administrator')}">
				<div align="center">
					<a href="xemdonhangdahuy"><input type="button" class="btn btn-default" value="Xem các đơn hàng đã hủy"/></a>
